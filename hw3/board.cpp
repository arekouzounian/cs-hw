...
Board::Board(const Board& b) :
    vehicles_(b.vehicles_), grid_(b.grid_), moveStack(Stack<MovePair>())
{
}
...
bool Board::move(VID_T vid, int amount)
{
    if (isLegalMove(vid, amount))
    {
        ...
        updateBoard();
        moveStack.push(std::make_pair(vid, amount));
        return true;
    }
    else
    {
...
{
    if (moveStack.empty() || moveStack.size() < 1)
    {
        throw std::underflow_error("No moves to undo!");
    }

    MovePair mv = moveStack.top();
    moveStack.pop();

    Vehicle& v = vehicles_[mv.first];
    if (v.direction == Vehicle::VERTICAL)
    {
        v.startr -= mv.second;
    }
...
{
    // To avoid compile errors in skeleton - Replace this
    MovePairList moves = MovePairList();
    VehicleMap::const_iterator it;
    for (it = vehicles_.begin(); it != vehicles_.end(); ++it)
    {
        Vehicle currV = it->second;
        if (currV.direction == Vehicle::HORIZONTAL)
        {
            //left
            int cnt = 0;
            for (int i = currV.startc - 1; i >= 0; --i)
            {
                VID_T currChar = this->at(currV.startr, i);
                if (currChar != Vehicle::INVALID_ID)
                    break;
                moves.push_back(std::make_pair(currV.id, (++cnt) * -1));
            }

            //right
            cnt = 0;
            for (int i = currV.startc + currV.length; i < this->size(); ++i)
            {
                VID_T currChar = this->at(currV.startr, i);
                if (currChar != Vehicle::INVALID_ID)
                    break;
                moves.push_back(std::make_pair(currV.id, ++cnt));
            }
        }
        else
        {
            ...
            {
                VID_T currChar = this->at(i, currV.startc);
                if (currChar != Vehicle::INVALID_ID)
                    break;
                moves.push_back(std::make_pair(currV.id, (++cnt) * -1));
            }
            ...
            //right
            cnt = 0;
            for (int i = currV.startr + currV.length; i < this->size(); ++i)
            {
                VID_T currChar = this->at(i, currV.startc);
                if (currChar != Vehicle::INVALID_ID)
                    break;
...
{
    if (this->vehicles_.size() != other.vehicles_.size())
    {
        return this->vehicles_.size() < other.vehicles_.size();
    }
    else
    {
        string lft = "";
        ...
        string rght = "";
        for (int i = 0; i < other.grid_.size(); ++i)
        {
            for (int j = 0; j < other.grid_[i].size(); ++j)
            {
                rght += other.grid_[i][j];
            }
        }
        return lft > rght;
    }
