...
struct LinearProber : public Prober<KeyType> {
    ...
    HASH_INDEX_T next()
    {
        ...
        { //check that we've come back to start after full wrap-around
            return this->npos;
        }
...
HashTable<K, V, Prober, Hash, KEqual>::HashTable(
    ...
    : hash_(hash), kequal_(kequal), prober_(prober), itemCount(0), resizeAlpha(resizeAlpha)
{
...
void HashTable<K, V, Prober, Hash, KEqual>::insert(const ItemType& p)
{
    ...
    double newAlpha = ((double)nonNullItemCount / table_.size());
    if (newAlpha >= resizeAlpha)
    {
        resize();
    }
...
void HashTable<K, V, Prober, Hash, KEqual>::remove(const KeyType& key)
{
    ...
    {
        if (!table_[loc]->deleted && table_[loc]->item.first == key) //we've found what we want
        {
...
void HashTable<K, V, Prober, Hash, KEqual>::resize()
{
    ...
    {
        throw std::logic_error("No more capacities exist to resize to!");
    }
    // O(m)
    std::vector<HashItem*> newTable = std::vector<HashItem*>();
    ...
    // O(table_.size()), which is guaranteed to be less than m
