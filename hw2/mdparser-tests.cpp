...
doMDParseCheck(const StrVec& test_patterns, const StrVecVec& expected_terms, const StrVecVec& expected_outgoing);
...
doMDParseCheck(const StrVec& test_patterns, const StrVecVec& expected_terms, const StrVecVec& expected_outgoing) {
    for (size_t i = 0U; i < test_patterns.size(); i++) {
        ...
        for (string t : expected_terms[i]) {
            bool found
                    = (terms.end() != terms.find(t))
                      || (terms.end() != terms.find(conv_to_lower(t)) || (terms.end() != terms.find(conv_to_upper(t))));
            if (!found) {
                return testing::AssertionFailure()
                       << " Couldn't find " << t << "(or an upper/lower case variation) in your terms";
            }
            terms.erase(t);
        ...
        if (0U != terms.size()) {
            return testing::AssertionFailure()
                   << terms.size() << " additional terms were returned from parsing beyond those expected.";
        }
        ...
        if (0U != outgoing.size()) {
            return testing::AssertionFailure()
                   << outgoing.size() << " additional outgoing links were returned from parsing beyond those expected.";
        }
    }
...
TEST(Parsers, MdParserSpecialStart) {
    ...
    std::vector<std::vector<std::string>> expected_terms = {{"Computer"}, {"science", "104"}};
...
TEST(Parsers, MdParserSpecialMiddle) {
    ...
    std::vector<std::vector<std::string>> expected_terms = {{"Computer", "Science"}, {"104", "103"}};
    std::vector<std::vector<std::string>> expected_outgoing = {{}, {}};
...
TEST(Parsers, MdParserLinksSepStart) {
    ...
    std::vector<std::vector<std::string>> expected_terms = {{"link1"}, {"link2", "link3"}};
    std::vector<std::vector<std::string>> expected_outgoing = {{"folder/page.txt"}, {"pagea.md"}};
...
TEST(Parsers, MdParserLinksSepMiddle) {
    ...
    std::vector<std::vector<std::string>> expected_terms = {{"link0", "link1"}, {"link2", "link3", "link4"}};
    std::vector<std::vector<std::string>> expected_outgoing = {{"folder/page.txt"}, {"pagea.md"}};
...
TEST(Parsers, MdParserLinksFull) {
    ...
    std::vector<std::vector<std::string>> expected_terms = {{"lInK1"}, {"text", "link"}};
    std::vector<std::vector<std::string>> expected_outgoing = {{"folder/page.txt"}, {"pagea.md"}};
...
TEST(Parsers, MDWriteupCornerCases) {
    ...
    std::vector<std::vector<std::string>> expected_terms
            = {{"empty", "url", "afterwards"}, {"no", "closing", "square", "bracket"}, {"some", "text"}, {}};
    std::vector<std::vector<std::string>> expected_outgoing = {{}, {}, {"url-to-nonexistent-file"}, {"some-file"}};
