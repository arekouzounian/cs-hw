...
private:
    ...
    std::vector<WebPage*> webpages_;
    std::map<std::string, WebPageSet> termMap_;

    /**
     * Given a filename, checks if it exists in webpages list
     *  and adds it if not
     *
     * @param[in] filename
     *  The file name/URL of the web page.
     *
     * @return A pointer to the web page, either newly created or existing.
     */
    WebPage* getWebPage(std::string filename);
};
